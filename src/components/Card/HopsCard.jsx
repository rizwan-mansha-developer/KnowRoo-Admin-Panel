...
import { deleteHops } from '../../redux/deleteSlices/HopsDeleteSlice';
...
const HopsCard = ({ count, id, hopsName, hopsDetails, Classes, CurrentStatus }) => {
  ...
  const handleConfirmDelete = () => {
    dispatch(deleteHops(selectedCardId));
  };
  ...
  const handleButtonClick = () => {
    navigate('/mobs/adventures/leaps/hops/startHops', { state: { id } });
    Cookies.set('hops_Id', id, { expires: 7 });
  };
    ...
    >
      <CardContent>
        <Grid style={{ display: 'flex' }}>
          <CountBox color={randomColor}>{count}</CountBox>
          <Typography
            variant="h4"
            component="div"
        ...
        <Typography
          ...
          style={{ fontSize: '14px', marginLeft: '0px', color: '#8C9FAA' }}
        >
          {truncateText(hopsDetails, CHARACTER_LIMIT)}
        </Typography>
        <FullWidthButton
          style={buttonStyles}
          endIcon={<ArrowIconBtn />}
